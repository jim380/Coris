...
export class ValidatorComponent implements OnInit {
  ...
  private selectedValidator: Validator | null = null;
  ...
  selectValidator(): void {
    ...
    this.appStore.select(selectValidators)
      .pipe(
        skipWhile(vals => vals.length === 0), 
        take(1))
      .subscribe(validators => {
        let filteredValidator = validators.filter(validator => validator.operator_address === this.address);

        if(filteredValidator.length === 1) this.selectedValidator = filteredValidator[0];
      });
